...
Class to convert the *.datx files generated by Zygo Fizeau instrument to *.h5 file. A measurement has the following data
1. Raw subapertures (2D images) from different scans  (n scans x m subapertures)
2. Header metadata

@author: adapa
@adapted: Murilo Bazan da Silva
"""
import datetime
import os
...
import numpy as np
from PyLOSt.data_in.fizeau.datx.read_datx import readDatxFile, readDatxMatrixSize
...
class DatxToH5():
    ...
    def toH5(self, in_folder, prefix, patch_count, h5scan, dircn, scanNo, scanIdx=1, nbSelScans=1, patches_selected=[],
             options=['', '-P', 'datx', []], progress_bar=None):
        """
        Function calls *.datx reader (read_datx.readDatxFile) and saves the data to h5 measurement entry
        ...
        try:
            ...
            for z in (reversed(patches_selected) if dircn == 'B' else patches_selected):
                fin_path = os.path.join(in_folder, joinFileNameSequence(prefix, scanNo, z, options))
                b = readDatxMatrixSize(fin_path)
                ox_aux.append(b['cn_org_x'])
                oy_aux.append(b['cn_org_y'])
                ex_aux.append(b['cn_org_x'] + b['cn_width'])
                ey_aux.append(b['cn_org_y'] + b['cn_height'])

            ox_min = np.amin(ox_aux)
            oy_min = np.amin(oy_aux)
            ex_max = np.amax(ex_aux)
            ey_max = np.amax(ey_aux)
            ...
            for j in (reversed(patches_selected) if dircn == 'B' else patches_selected):
                ...
                d = readDatxFile(fin_path)
                # ox = d['cn_org_x']
                # oy = d['cn_org_y']
                # wx = d['cn_width']
                # wy = d['cn_height']

                ox = d['cn_org_x'] - ox_min
                oy = d['cn_org_y'] - oy_min
                wx = d['cn_width']
                wy = d['cn_height']

                # h = np.full((d['ac_height'],d['ac_width']), np.nan, dtype='float32')
                # h = np.full((d['camera_height'][0],d['camera_width'][0]), np.nan, dtype='float32')
                h = np.full((ey_max - oy_min, ex_max - ox_min), np.nan, dtype='float32')
                ...
                # if np.size(d['data_intensity']) == 0:
                intensity_patches.append(np.ones_like(h))
                # else:
                #    intensity_patches.append(d['data_intensity'])
                ...
                # h[h>2e7]=np.nan
                print(prefix + str(scanNo) + '-P' + str(j) + '.datx')
            data_shape = np.array(hgt_patches).shape
            intensity_data_shape = np.array(intensity_patches).shape
            # slopes in mrad        
            ...
            data.attrs[u'pupil_data_dimensions'] = (d['camera_height'][0], d['camera_width'][0])
            ...
            data.create_dataset('intensity', shape=intensity_data_shape, dtype='int8', chunks=True)
